...
do_who(struct Client *source_p, struct Client *target_p, struct membership *msptr, struct who_format *fmt)
{
    ...
    rb_sprintf(status, "%c%s%s%s",
               target_p->user->away ? 'G' : 'H', IsSetBot(target_p) ? "B" : "", IsOper(target_p) ? "*" : "", msptr ? find_channel_status(msptr, fmt->fields || IsCapable(source_p, CLICAP_MULTI_PREFIX)) : "");
